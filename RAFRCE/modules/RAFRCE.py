...
class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat):
        ...
        self.gate_1 = nn.Linear(self.emb_size * 2, 1)
    ...
    def forward(
        ...
        i_e_3 = self.gate(i_e, i_e_2)
        # u_e_3 = self.gate(u_e, u_e_2)
        loss_contrast = self.alpha * self.calculate_loss(i_e_1, i_e)

        u_e = torch.cat((u_e, u_e_2), dim=-1)
        i_e = torch.cat((i_e_3, i_e_1), dim=-1)
    ...
    def gate(self, x1, x2):
        ...
        gate_weight = torch.sigmoid(self.gate_1(combined))

        aggregated = gate_weight * x1 + (1 - gate_weight) * x2
        return aggregated
    ...
    def create_bpr_loss(self, users, items, labels, loss_contrast):
        ...
        return bce_loss + emb_loss + 0.1 * loss_contrast, scores, bce_loss, emb_loss
